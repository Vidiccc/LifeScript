...
public class KhachHangController extends HttpServlet {
			...
			if (baoLoi.length() > 0) {
				...
				if (khachHangDAO.insert(kh) > 0) {
					...
					if(khachHangDAO.updateVerifyInformation(kh)>0) {
						...
						Email.sendEmail(kh.getEmail(), "Xác thực tài khoản tại LifeScripts.com", getNoiDung(kh));
					}
				}
	...
	public static String getNoiDung(KhachHang kh) {
		String link = "http://localhost:8080/LifeScripts/khach-hang?hanhDong=xac-thuc&maKhachHang="+kh.getMaKhachHang()+"&maXacThuc="+kh.getMaXacThuc();
		String noiDung = "<p>LifeScripts xin ch&agrave;o bạn <strong>"+kh.getHoVaTen()+"</strong>,</p>\r\n"
